...
버전 `0.9.6-quality-gates`. 게임 개발 28개와 UI 11개를 **한 묶음의 39개 스킬**로 관리합니다. UI도 `skills/`에 포함하므로 별도 UI 저장소나 설치가 필요하지 않습니다. `game-ui-art-direction`에는 독립 검증된 선택용 모션·버튼 형태 샘플 라이브러리가 포함됩니다.

## 구성과 사용

- `game-workflow`가 새 요청과 재개 요청을 먼저 연결·분류하고, 상태가 있거나 여러 단계인 작업은 전문 스킬 전에 하나의 supervisor로 넘깁니다. 명시한 전문 스킬 선택은 유지합니다.
- 재개할 때 최신 사용자 지시와 기존 실행 기록의 권한을 대조합니다. 오래된 차단·승인 상태를 그대로 실행 권한으로 쓰지 않으며, 명시적인 재개가 별도 외부 작업의 승인까지 만들지는 않습니다.
...
- `game-ui-ux-design`이 플레이어 작업 흐름·정보 우선순위·상호작용 피드백을 정리하고 화면 명세·컴포넌트·목업으로 연결합니다. 호환되는 기존 UX 결정은 재사용하며 페르소나, 샘플 선택, 정식 캡처 사전검증을 유지합니다.
- 정식 목업은 같은 후보 지문·전체 선택 범위에서 계약 준수와 시각 결정 준비도를 작성자와 별도 검증자가 각각 판정합니다. 아트 방향도 비교 무결성과 시각 완성도를 분리하고, 고비용 다중 상태·대상 작업은 대표 위험 화면의 독립 preflight를 통과한 뒤 전체 패킷으로 확장합니다.
- `game-workflow-audit`는 반복 수리 실패(T1), 품질 반려(T2), 요구와 모순된 PASS(T3), 기록된 예산 대비 무진전(T4)에 반드시 별도 배정되며 정상 회고는 선택적으로 유지합니다. 감사가 제품이나 스킬 수정 권한을 만들지는 않으며, 수정과 별도 검증 결과는 사용자가 이해할 수 있는 전후 변화로 보고합니다.
...
첫 명령은 미리보기, 두 번째는 39개 스킬과 참조·양식·도구 설치입니다. 설치기는 오프라인이며 모든 입력을 이 묶음의 `skills/`에서만 읽고 `bundle.lock.json`의 전체 파일 해시를 확인합니다.
