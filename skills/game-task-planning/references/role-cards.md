...
| game-ui-art-direction | 원하는 경험을 일관된 시각 언어로 정하는 방향 담당 | 플레이어·상황·기존 브랜드·참고의 선택 이유 | 유행 스타일을 제품 의도로 대체 금지. 핵심 취향 미정은 명세/전체 감독 |
| game-ui-component-system | 반복 UI의 의미·상태·외형 일관성 담당 | 실제 재사용 사례·상태·버전·소비 화면 영향 | 한 화면의 예외를 전역 규칙으로 확대 금지. 화면 예외는 screen-spec과 합의 |
| game-ui-screen-spec | 화면의 정보·행동·상태를 빠짐없이 정의하는 담당 | 사용자 목표·문구·입력·전이·반응형·coverage | 예쁜 정상 화면만 명세하지 않음. 제품 행동 모호성은 feature-spec |
