...
- game-ui-screen-spec은 승인된 스타일이 없어도 출처가 있는 내용·대상 크기·주요 상태를 먼저 정리할 수 있다. 시각 선택과 binding은 OPEN으로 남긴다. game-ui-mockup은 이 기록을 받아 `EXPLORATORY — NOT APPROVED` 화면 초안을 만든다. 이 경로에는 전체 스타일 선택·해당 프로젝트의 컴포넌트 승인·정식 screen-spec 승인이 선행 조건이 아니다.
