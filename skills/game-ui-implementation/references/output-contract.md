...
Read the rules for the selected mode below. Exact headers, tables, approvals and recovery routes in the full-integration contract bind full integration; the playable functional interface returns its separate compact handoff.

## Playable functional interface
For the functional mode in [game-ui-implementation](../SKILL.md), return a compact functional handoff: selected mode and code-impact decision, approved action/state contract and target, changed source and candidate identity, real-data wiring, focused player-loop scenarios with actual-runtime input/result evidence, deferred appearance, and the owner of final polish. A separate functional verifier checks the agreed interactions and outcomes on the current candidate. Record incomplete or unusable steps as such; core-only checks and browser mockups cannot pass this gate. This handoff is not an implementation evidence packet, Component runtime integration matrix, asset readiness or visual-fidelity verdict. Formal UI production later follows its own approved inputs and current runtime acceptance. If polish changes behavior, hit geometry, state meaning or shared dependencies, rerun the affected functional scenarios.

## Code-native runtime-source preparation before asset readiness

For the bounded mode in [game-ui-implementation](../SKILL.md), produce a source-preparation result, not an implementation evidence packet or `READY` integration matrix. Lock the approved handoff/catalog binding and fingerprint, exact declared adapter contract and intended source locator, allowed inputs, protected properties, required states, target, fixture and intended consumer. Record the source's pre-state as its actual hash or `ABSENT` when uncreated; never invent a pre-hash. Record changed source/fixture files and their produced hashes, focused checks, and actual isolated target-size component renders with original build/source identity before separate verification and asset parity. A separate verifier checks this scoped result; game-ui-asset-production determines source parity and packet readiness. Missing actual source or target-size render remains `BLOCKED`; missing or conflicting declared contract/locator blocks preparation. The preparation neither authorizes screen integration nor claims runtime validation or visual acceptance.
...
Before returning the full implementation packet, compare the declared code-native component source hash and relevant parity fixture/render dependencies with the exact source and isolated target-size parity evidence accepted by asset production. Record those compared identities and the separate integrated build identity in the implementation packet; a changed whole-build hash alone is not source-parity drift. A changed declared representation, protected-property source, stale parity fixture or relevant fixture dependency makes the affected binding and packet readiness `BLOCKED`: return it to game-ui-asset-production to record the disposition, recheck isolated target-size parity and obtain independent verification, then repeat the affected integration checks on the restored ready packet. A screen composition, live-content or input-only change that leaves these pre-integration inputs intact needs affected runtime validation, not another asset packet. Keep unaffected evidence only where the impact rules support it. Do not dispatch runtime validation while any required binding or packet is blocked.
